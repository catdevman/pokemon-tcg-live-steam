...
- Open Steam, Library, right-click on `Pokemon TCG Live` and select Properties under Shortcut you'll see the name you change earlier and `Target` and `Start In`, for Target using quotes `"/home/<username>/.local/share/Steam/steamapps/compatdata/<appid>/pfx/drive_c/users/steamuser/The Pokemon Company International/Pokemon Trading Card Game Live/Pokemon TCG Live.exe"` and for Start In it'll look like `"/home/<username>/.local/share/Steam/steamapps/compatdata/<appid>/pfx"` in quotes, exit from Properties.
